...
open class GuillotineTransitionAnimation: NSObject {
    ...
    fileprivate func showHostTitleLabel(_ show: Bool, animated: Bool) {
        ...
        //要保证旋转后，标题栏和导航栏的标题能重叠，supportView就是NavigationBar
    ...
    func setupContainerMenuButtonFrameAndTopOffset() {
        ...
        var senderRect = supportView!.convert(presentButton!.frame, to: nil)
        senderRect.origin.x = senderRect.origin.x + 16
        senderRect.origin.y = senderRect.origin.y + 8
